class Api::PosNegLeverageController < ApplicationController
  ...
  def update
    ...
    @pos_neg_leverage.purchase_price = params[:pos_neg_leverage][:purchase_price].gsub(',', '').gsub(' ','')
    @pos_neg_leverage.cap_rate = params[:pos_neg_leverage][:cap_rate].gsub(',', '').gsub(' ','')
    @pos_neg_leverage.loan_amount = params[:pos_neg_leverage][:loan_amount].gsub(',', '').gsub(' ','')
    @pos_neg_leverage.low_interest_rate = params[:pos_neg_leverage][:low_interest_rate].gsub(',', '').gsub(' ','')
    @pos_neg_leverage.high_interest_rate = params[:pos_neg_leverage][:high_interest_rate].gsub(',', '').gsub(' ','')
    @pos_neg_leverage.interest_only = (params[:pos_neg_leverage][:interest_only].downcase == "yes")
    @pos_neg_leverage.amortization = params[:pos_neg_leverage][:amortization].gsub(',', '').gsub(' ','')
